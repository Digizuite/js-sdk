...
export interface IUpdateContainerJson {
    ...
    ItemIds: number[];
    RowId: number;
    Values: { FieldId: (string | any); Type: (string | number | any); Values: Array<any> }[]
}
...
export class UpdateContainer {
	...
	constructor(args: { type?: number, itemIds?: number[], id?: string, fieldName?: string, rowId?: number | string }) {
        ...
        if (args.hasOwnProperty('rowId')) {
            let parsedRowId = parseInt(<string><any>args.rowId, 10);
     ...
     */
    getContainerJSON(): IUpdateContainerJson {
        return {
            Id: this.containerId,
